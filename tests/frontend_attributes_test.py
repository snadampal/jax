...
class FrontendAttributesTest(jtu.JaxTestCase):
  ...
  def test_f_jitted_mlir(self):
    ...
    def f_string(a, b):
      with set_xla_metadata(a="b"):
        return a + b
    self.assertIn('mhlo.frontend_attributes = {a = "b"}',
                  f_string.lower(1.0, 2.0).as_text())

    @jax.jit
    def f_bool(a, b):
      with set_xla_metadata(a=True):
        return a + b
    self.assertIn('mhlo.frontend_attributes = {a = "true"}',
                  f_bool.lower(1.0, 2.0).as_text())

    @jax.jit
    def f_int(a, b):
      with set_xla_metadata(a=10):
        return a + b
    self.assertIn('mhlo.frontend_attributes = {a = "10"}',
                  f_int.lower(1.0, 2.0).as_text())
    ...
    with set_xla_metadata(a="b"):
      ...
      def f(a, b):
        with set_xla_metadata(a="c"):
          return a + b
      # f gets the innermost scope
      self.assertIn('mhlo.frontend_attributes = {a = "c"}', f.lower(1.0, 2.0).as_text())
      # g called in scope of a="b"
      self.assertIn('mhlo.frontend_attributes = {a = "b"}', g.lower(1.0, 2.0).as_text())

    # No attributes on g outside of the scope
    ...
    with set_xla_metadata(key1="val1"):
      ...
      def f(a, b):
        with set_xla_metadata(key2="val2"):
          return a + b
      f_lowered_text = f.lower(1.0, 2.0).as_text()
      # if attributes with different keys are present, they are merged
      self.assertIn(
  ...
  def test_attr_caching_nonjit_mlir(self):
    ...
    # Expect no attributes in the jaxpr outside the scope.
    self.assertNotIn(
        "mhlo.frontend_attributes",
        jax.jit(f_add).lower(arg2, arg2).as_text(),
    )

   # Expect recompilation with different attributes.
    with set_xla_metadata(c="d"):
      self.assertIn(
